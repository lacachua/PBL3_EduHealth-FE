  ...
  return (
    <>
      <div
        aria-hidden="true"
        onClick={onClose}
        className={`fixed inset-0 z-30 bg-on-surface/30 transition md:hidden ${isSidebarOpen ? 'opacity-100' : 'pointer-events-none opacity-0'
          }`}
      />

      <aside
        className={`fixed inset-y-0 left-0 z-40 flex ${sidebarWidthClasses.drawer} flex-col overflow-x-hidden border-r border-outline-variant bg-surface shadow-[2px_0_10px_rgba(15,23,42,0.04)] transition-[transform,width] duration-200 md:translate-x-0 ${sidebarWidthClass} ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
        <div className={`flex items-center border-b border-outline-variant/80 py-3.5 ${isSidebarCollapsed ? 'justify-center gap-2 px-2' : 'gap-2 px-3.5'
          }`}>
          {isSidebarCollapsed ? (
        ...
        <nav className="flex-1 space-y-5 overflow-y-auto px-2.5 pb-3.5 pt-3.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {adminSidebarGroups.map((group) => (
            <div key={group.id}>
              <p className={`px-3.5 pb-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-on-surface-variant/80 ${isSidebarCollapsed ? 'hidden' : ''
                }`}>
                {group.label}
              </p>
                ...
                {group.items.map((item) => {
                  return (
                    <NavLink
                      ...
                      className={({ isActive }) => `group relative flex items-center rounded-xl px-3.5 py-2.5 text-[15px] font-semibold transition ${isSidebarCollapsed ? 'justify-center' : ''
                        } ${isActive
        ...
        <div className="border-t border-outline-variant/80 px-2.5 py-3">
          <button
            ...
            className={`inline-flex w-full items-center rounded-xl border border-transparent text-sm font-semibold text-on-surface-variant transition hover:border-outline-variant hover:bg-surface-container-low ${isSidebarCollapsed ? 'justify-center px-2 py-2.5' : 'gap-2 px-3.5 py-2.5'
              }`}
          >
