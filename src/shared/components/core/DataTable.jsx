const DataTable = ({
  ...
  return (
    <div
      className={
        containerClassName ||
        'overflow-x-auto rounded-2xl border border-outline-variant bg-surface [scrollbar-width:thin]'
      }
    >
      <table className={tableClassName || 'min-w-full text-left text-sm'}>
        <thead
          className={
            headClassName || 'app-table-head text-[11px] uppercase tracking-[0.08em]'
          }
        >
          <tr>
            {columns.map((column) => (
              <th
                key={column.key}
                className={`${headCellPaddingClass} ${column.headerClassName || ''}`}
              >
                {column.header}
              </th>
            ))}
          ...
          {rows.map((row, index) => (
            ...
            >
              {columns.map((column) => (
                <td
                  key={column.key}
                  className={`align-top ${bodyCellPaddingClass} ${column.cellClassName || ''}`}
                >
