...
const cards = [
  ...
  { id: 'bmi', label: 'BMI', icon: 'monitor_heart', unit: 'kg/m2', key: 'bmi' },
      ...
      {cards.map((card) => (
        <article key={card.id} className="app-card-shell rounded-xl px-3.5 py-3">
          ...
          <div className="flex items-end gap-1.5">
            ...
            {card.unit ? <span className="pb-0.5 text-[11px] font-medium text-[#64748B]">{card.unit}</span> : null}
          </div>

          <p className="mt-1 text-[11px] text-[#64748B]">{card.id === 'bmi' ? (metrics?.note || '--') : 'Cập nhật từ hồ sơ sức khỏe hiện tại'}</p>
        </article>
      ))}
    </section>
  );
