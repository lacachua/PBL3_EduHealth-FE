...
const buildInitialForm = (medicine) => ({
  name: medicine?.name || '',
  activeIngredient: medicine?.activeIngredient || '',
  unit: medicine?.unit || 'VIEN',
  packaging: medicine?.packaging || '',
  warningThreshold: medicine?.warningThreshold || 1,
  note: medicine?.note || '',
});

const UpdateMedicineModal = ({ open, medicine, onClose, onSubmit, submitting, error }) => {
  const [form, setForm] = useState(buildInitialForm(medicine));

  const modalTitle = useMemo(() => {
    if (!medicine?.name) return 'Cập nhật thuốc';
    return `Cập nhật: ${medicine.name}`;
  }, [medicine?.name]);
  ...
  return (
    <InventoryActionModal
      open={open}
      title={modalTitle}
      subtitle="Chỉnh sửa thông tin cơ bản của thuốc"
      error={error}
      onClose={onClose}
