  ...
  return (
    <div ref={containerRef} className="relative inline-flex justify-end">
      <button
        ...
        className={`inline-flex h-8 w-8 items-center justify-center rounded-md border bg-[#FBFCFB] transition hover:bg-[#F3F8F6] ${STUDENT_BASE_CLASS.border} ${STUDENT_BASE_CLASS.mutedText} hover:text-[#42534D]`}
      ...
      {open ? (
        <div className={`absolute right-0 top-9 z-30 min-w-[180px] rounded-md border bg-[#FBFCFB] p-1 shadow-[0_8px_18px_rgba(15,23,42,0.1)] ${STUDENT_BASE_CLASS.border}`}>
          {items.map((item) => (
            <button
              key={item.id}
              type="button"
