  ...
  return (
    ...
    >
      <div className="p-4 pt-3">
        <p className="app-overline mb-2">Phân tích theo nhóm triệu chứng</p>
        {!items.length ? (
          <EmptyState
              ...
              <div className="relative mx-auto h-32 w-32 shrink-0 rounded-full" style={{ background: pieGradient }}>
                <div className="absolute inset-[16px] flex items-center justify-center rounded-full bg-surface text-center">
                  <div>
                    ...
                    <p className="text-[1.78rem] font-extrabold leading-none text-on-surface">{totalCases}</p>
                  </div>
                </div>
              </div>
                ...
                {items.map((item) => (
                  <div key={item.id} className="rounded-lg border border-outline-variant bg-surface px-2.5 py-2">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-[12px] font-semibold text-on-surface-variant">{item.count} ca ({item.ratio}%)</span>
                    </div>
