...
const ClassBar = ({ item }) => {
  ...
  const stablePct = Number.isFinite(item.stablePct) ? item.stablePct : (total ? Math.round((item.stable / total) * 100) : 0);
  const followUpPct = Number.isFinite(item.followUpPct) ? item.followUpPct : (total ? Math.round((item.followUp / total) * 100) : 0);
  const highRiskPct = Number.isFinite(item.highRiskPct) ? item.highRiskPct : (total ? Math.round((item.highRisk / total) * 100) : 0);
...
const AdminReportMainChart = ({ data, meta }) => {
  ...
  return (
    <section className="rounded-3xl border border-outline-variant bg-surface-container-lowest p-8 shadow-sm">
      <div className="mb-10 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-5 rounded-xl border border-outline-variant bg-surface p-2.5">
      ...
      <div className="relative flex h-72 items-end gap-8 border-b border-outline-variant/50 pb-2 pl-8 pr-4">
